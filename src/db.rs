...
pub async fn run_migrations(pool: &PgPool) -> AppResult<()> {
    ...
    // Ensure MDR integration schema is present for runtime-managed environments.
    sqlx::query(
        r#"
    ...
    .execute(pool)
    .await
    .map_err(|e| AppError::DatabaseError(format!("Failed to create clients table: {}", e)))?;

    sqlx::query(
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_clients_name_branch ON clients (UPPER(name), COALESCE(branch, ''))",
    )
    .execute(pool)
    .await
    .map_err(|e| AppError::DatabaseError(format!("Failed to create clients index: {}", e)))?;
    ...
    .await
    .map_err(|e| {
        AppError::DatabaseError(format!("Failed to create mdr_import_batches table: {}", e))
    })?;
    ...
    .await
    .map_err(|e| {
        AppError::DatabaseError(format!("Failed to create guard_assignments table: {}", e))
    })?;

    for index_sql in &[
        "CREATE INDEX IF NOT EXISTS idx_guard_assignments_guard ON guard_assignments(guard_id)",
        "CREATE INDEX IF NOT EXISTS idx_guard_assignments_client ON guard_assignments(client_id)",
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_guard_assignments_active ON guard_assignments(guard_id, client_id, assignment_start) WHERE status = 'active'",
    ] {
        sqlx::query(index_sql)
            .execute(pool)
            .await
            .map_err(|e| {
                AppError::DatabaseError(format!("Failed to create guard_assignments index: {}", e))
            })?;
    }
    ...
    .map_err(|e| {
        AppError::DatabaseError(format!(
            "Failed to create guard_status_transitions table: {}",
            e
        ))
    })?;

    sqlx::query(
        "CREATE INDEX IF NOT EXISTS idx_guard_transitions_guard ON guard_status_transitions(guard_id, created_at DESC)",
    )
    .execute(pool)
    .await
    .map_err(|e| {
        AppError::DatabaseError(format!("Failed to create guard_status_transitions index: {}", e))
    })?;
    ...
    .execute(pool)
    .await
    .map_err(|e| AppError::DatabaseError(format!("Failed to create equipment table: {}", e)))?;
    ...
    sqlx::query(
        r#"
        CREATE TABLE IF NOT EXISTS armored_car_status_transitions (
            id VARCHAR(36) PRIMARY KEY,
            car_id VARCHAR(36) NOT NULL,
            previous_status VARCHAR(50) NOT NULL,
            new_status VARCHAR(50) NOT NULL,
            reason TEXT,
            mdr_batch_id VARCHAR(36),
            mdr_row_ref VARCHAR(50),
    ...
    .map_err(|e| {
        AppError::DatabaseError(format!(
            "Failed to create armored_car_status_transitions table: {}",
            e
        ))
    })?;

    for index_sql in &[
        "CREATE INDEX IF NOT EXISTS idx_armored_car_status_transitions_car_created ON armored_car_status_transitions(car_id, created_at DESC)",
        "CREATE INDEX IF NOT EXISTS idx_armored_car_status_transitions_batch_created ON armored_car_status_transitions(mdr_batch_id, created_at DESC)",
    ] {
        sqlx::query(index_sql)
            .execute(pool)
            .await
            .map_err(|e| {
                AppError::DatabaseError(format!(
                    "Failed to create armored_car_status_transitions index: {}",
                    e
                ))
            })?;
    }
